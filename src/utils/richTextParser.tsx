...
import { isBlockquote, isCode, isHeading, isList, isParagraph } from 'datocms-structured-text-utils';
...
import ImageZoom from 'molecules/ImageZoom';
...
const richTextParser = (content: RichText, showLineNumbers?: boolean, useInlineStyle?: boolean) => {
  ...
  const customNodeRules = [
    ...
    renderNodeRule(isList, ({ node, children, key }) =>
      node.style === 'bulleted' ? (
        <Flex as="ul" paddingLeft={0} gap="16px" flexDirection="column" key={key}>
          {children?.map(child => (
            <Flex
              key={child.key}
              as="li"
            ...
            >
              <Flex flex="0 0 16px" paddingTop={2}>
                <Icon iconName="check-circle-4" size={20} color={color.success[300]} />
              </Flex>
              {child}
            </Flex>
          ))}
      ...
      renderInlineRecord={({ record }) => {
        switch (record.__typename) {
          case 'ComponentImageRecord':
            return <ImageZoom image={record as unknown as IComponentImageRecord} />;
          case 'ComponentVideoRecord':
            return <ComponentVideo {...(record as unknown as IComponentVideoRecord)} />;
          case 'ComponentFormRecord':
            return <ComponentForm {...(record as unknown as IComponentFormRecord)} />;
          case 'MenuItemRecord':
